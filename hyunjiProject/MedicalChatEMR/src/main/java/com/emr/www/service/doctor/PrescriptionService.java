...
public class PrescriptionService {
	...
	public List<PrescriptionDTO> searchPrescriptions(String query) {
		int pageNo = 1;
		int startPage = 1;
		int numOfRows = 3; // 페이지당 결과 수를 원하는 대로 조정
		String type="xml";

		String xmlResponse = callPrescriptionDirectly(query, pageNo, numOfRows, type);
		return parseXmlToPrescriptionList(xmlResponse);
	}
	...
	private String callPrescriptionDirectly(String query, int pageNo, int numOfRows, String type) {
		...
		String apiUrl = String.format("%s?serviceKey=%s&itemName=%s&pageNo=%s&numOfRows=%s&type=%s", apiBaseUrl, apiKey, encodedQuery,
	...
	//XML 응답을  DTO 목록으로 변환
	private List<PrescriptionDTO> parseXmlToPrescriptionList(String rawXmlResponse){
