...
<body>
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            console.log("로드된 콘텐츠:", data);

            // 동적으로 로드된 콘텐츠 안에 포함된 스크립트를 실행
            const scripts = document.querySelector('.board').getElementsByTagName('script');
            for (let script of scripts) {
                eval(script.innerText);  // 스크립트 실행
            }
            
            // 동적으로 로드된 스크립트 실행
            if (typeof initEmployeeForm === "function") {
                console.log("initEmployeeForm 함수 실행");
                initEmployeeForm();
            }
            if (typeof initFormSubmission === "function") {
                console.log("initFormSubmission 함수 실행");
                initFormSubmission();
            }
        })
